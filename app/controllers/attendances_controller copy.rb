...
#          debugger
  ...
  def edit_notice_overtime
    ...
    @notice_users = User.where(id: Attendance.where(confirmation_id_zangyou: @users.id).select(:user_id))
  ...
  def edit_attendance_up
    ...
    @attendances2 = Attendance.where(confirmation_id: params[:user_id]).where(confirmation_change_status: "申請中")
    @notice_users = User.where(id: @attendances2.select(:user_id))
    @attendances_list = Attendance.where.not(user_id: params[:id])
    @notices1 = @attendances_list.where.not(confirmation_change: @users.name)
    @notices = @notices1.where(change_attendance: nil).or(@notices1.where(confirmation_change_status: "申請中"))
